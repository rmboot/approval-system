...
from approval_system.forms import LoginForm, RegisterForm, RegisterAdminForm, SettingForm, PasswordForm
...
def register():
    ...
    if form.validate_on_submit():
        ...
        password = form.password.data
        user = User(number=number, name=name, dept_id=dept_id, phone=phone, role_id=1)
...
def register_admin():
    ...
    if form.validate_on_submit():
        ...
        phone = form.phone.data
        password = form.password.data
        user = User(number=number, name=name, dept_id=dept_id, role_id=role_id, phone=phone)
        user.set_password(password)
        db.session.add(user)
        db.session.commit()
        login_user(user)
